...
interface ChatAddUserFormProps {
  closeHandler: (value: string) => void
  switchHadler: (value: string) => void
}

export class ChatAddUserForm extends Block<ChatAddUserFormProps> {
  errors: { [key: string]: string }
  constructor(props: ChatAddUserFormProps) {
    super(props)
    this.errors = {}
  }
      ...
      events: {
        click: e => {
          ...
          if (!Object.keys(this.errors).length) {
            ...
            chatsController.addUsersToChat({ login: data.login })
            this.props.switchHadler('ModalAddUser')
            inputEl.value = ''
          }
        }
      }
